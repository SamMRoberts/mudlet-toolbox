...
3. Prefer parent-relative Geyser constraints. Add a resize event handler only for layout work the constraints cannot express. Reflow without recreating widgets or writing back the main window size.
...
5. Tear down event/timer producers before widgets. Choose cleanup supported by the target version; hiding is not deletion. Restore shared settings only under an explicit ownership arrangement.
